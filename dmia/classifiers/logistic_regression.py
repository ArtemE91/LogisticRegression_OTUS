...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        Inputs:
        - X: N x D массив обучающих данных. Каждая тренировочная
            точка является D-мернойколонкой.
        - y: Одномерный массив длины N с метками 0-1 для 2 классов.
        - learning_rate: (float) скорость обучения для автоматизации.
        - reg: (float) сила регуляризации.
        - num_iters: (integer) количество шагов при оптимизации
        - batch_size: (integer) количество обучающих примеров для использования
                    на каждом этапе.
        - verbose: (boolean) Если true, печатать прогресс во время оптимизации.
    ...
    @staticmethod
    def append_biases(X):
        # sparse.hstack - сложение разряженной матрицы горизонтально
        # X - последовательность разряженных матриц
        # np.ones - вернет массив заполненный 1
